...
DESCRIPTION = "Recipe to install a precompiled binary"
LICENSE = "CLOSED"
SRC_URI = " file://tinkersensors \
            file://tinkersensors.service "

inherit deploy systemd

do_install() {
    install -d ${D}${bindir}
    install -m 0755 ${UNPACKDIR}/tinkersensors ${D}${bindir}

    install -d ${D}${systemd_system_unitdir}
    install -m 0644 ${UNPACKDIR}/tinkersensors.service ${D}${systemd_system_unitdir}/
}

SYSTEMD_AUTO_ENABLE = "enable"
SYSTEMD_PACKAGES += "${PN}"
SYSTEMD_SERVICE:${PN} = "tinkersensors.service"

FILES_${PN} += " ${bindir}/tinkersensors \
                 ${systemd_system_unitdir}/tinkersensors.service"
